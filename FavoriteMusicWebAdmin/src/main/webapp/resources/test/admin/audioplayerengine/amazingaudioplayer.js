 ...
 * http://amazingaudioplayer.com Version 3.2
 */
	...
	$.fn.amazingaudioplayer = function(options) {
		var PlayerSkin = function(amazingPlayer, container, options, id) {
			...
			if (this.options.showimage) {
				...
				this.$image.css({
					width : this.options.imagefullwidth ? "100%;"
			...
			if (this.options.showtitleinbar) {
				...
				this.$bartitle.css({
					...
					overflow : "hidden",
					"white-space" : "nowrap",
					width : this.options.titleinbarwidthmode == "auto" ? "auto"
							: this.options.titleinbarwidth,
					height : "auto"
				});
				this.container
						.bind(
				...
				this.container
						.bind(
								...
								function(event, data) {
									...
									var t;
									if (data.live)
										t = instance.options.timeformatlive
			...
			if (this.options.showprogress) {
				...
				if ($progress.css("right") == "auto")
					$progress.css("right", $progress.css("left"));
				$progressLoaded.css({
					display : "block",
					position : "absolute",
					left : 0,
		...
		FlashHTML5Player.prototype = {
			...
			play : function() {
				if (this.isHtml5) {
					...
					this.flashLoadAudio(this.getMp3Src(), true);
				this.isPlaying = true
			},
			getMp3Src : function() {
			...
			setTime : function(pos, duration) {
				if (this.isHtml5)
					if (!isNaN(this.html5Object.get(0).duration)
						...
						this.html5Object.get(0).currentTime = duration * pos;
				else
		...
		AmazingAudioPlayer.prototype = {
			...
			playProgress : function(current, duration) {
				...
				var d0 = this.elemArray[this.currentItem].duration * 1E3 > duration
						|| isNaN(duration) || !isFinite(duration) ? this.elemArray[this.currentItem].duration * 1E3
						: duration;
				this.container.trigger("amazingaudioplayer.playprogress", {
					current : current,
					duration : d0,
		...
		this
				.each(function() {
					...
					if (typeof amazingaudioplayer_options != "undefined"
							&& amazingaudioplayer_options)
						this.options = $.extend(this.options,
								amazingaudioplayer_options);
					if ($("div#amazingaudioplayer_options").length)
						$.each($("div#amazingaudioplayer_options").data(),
								function(key, value) {
									instance.options[key.toLowerCase()] = value
								});
					var searchoptions = {};
